...
const EnregDressage = ({ navigation }) => {
  ...
  const handleCSO = async () => {
    ...
    const dateEpreuve = `${annee}-${mois}-${jour}`;
    try {
      const response = await axios.post('http://10.0.2.2:3000/enreg/dressage', {
        token,
    ...
    <ImageBackground source={bgImage} resizeMode="cover" style={styles.imageBackground}>
      <View style={styles.container}>
        <View style={styles.colorbg}>
          <Text style={styles.title}>Enregistrez votre concours de Dressage</Text>
